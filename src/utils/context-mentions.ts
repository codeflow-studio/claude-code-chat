...
/**
 * Represents a search result for files or folders
 */
...
/**
 * Inserts a mention directly at the current cursor position without checking for existing @
 */
...
/**
 * Removes a mention when the user presses backspace
 */
...
/**
 * Types of options available in the context menu
 */
...
/**
 * Interface for items that can be displayed in the context menu
 */
...
/**
 * Determines if the context menu should be shown based on the current text and cursor position
 */
...
/**
 * Gets the options to display in the context menu based on the query and available items
 */
export function getContextMenuOptions(
    ...
    // Simple filtering for the items
    ...
    const fileMatches = filteredItems.filter(
        (item) => item.type === ContextMenuOptionType.File || item.type === ContextMenuOptionType.Folder,
    );
    const gitMatches = filteredItems.filter((item) => item.type === ContextMenuOptionType.Git);
    const otherMatches = filteredItems.filter(
        (item) =>
    ...
    const searchResultItems = dynamicSearchResults.map((result) => {
        ...
        const item = {
            ...
            value: formattedPath,
            label: result.label || (formattedPath.split("/").pop() || ""),
    ...
    if (suggestions.length > 0 || filteredItems.length > 0) {
